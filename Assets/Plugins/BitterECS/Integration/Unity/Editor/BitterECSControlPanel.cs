...
namespace BitterECS.Integration.Unity.Editor
{
    ...
    public class EntitiesView
    {
        ...
        private readonly List<PrefabData> _allPrefabs = new();
        ...
        private readonly Dictionary<string, bool> _componentFoldouts = new();
        private readonly Dictionary<int, SerializedObject> _serializedObjects = new();

        private string TargetPath
        {
            get
            {
                string pName = !string.IsNullOrEmpty(PathProject.ProductName) ? PathProject.ProductName : "App";
                string rPath = !string.IsNullOrEmpty(PathProject.RootPath) ? PathProject.RootPath : "";
                string entities = !string.IsNullOrEmpty(PathProject.ENTITIES) ? PathProject.ENTITIES : "Entities";
                return $"Assets/!{pName}/{rPath}/{entities}".Replace("//", "/").Replace("\\", "/");
            }
        }
        ...
        public void OnEnable()
        {
            ...
            _owner.expandedEntityKeys?.ForEach(k => _entityFoldouts[k] = true);

            _componentFoldouts.Clear();
            _owner.expandedComponentKeys?.ForEach(k => _componentFoldouts[k] = true);
        ...
        public void SaveState()
        {
            ...
            _owner.expandedEntityKeys.AddRange(_entityFoldouts.Where(kvp => kvp.Value).Select(kvp => kvp.Key));

            _owner.expandedComponentKeys.Clear();
            _owner.expandedComponentKeys.AddRange(_componentFoldouts.Where(kvp => kvp.Value).Select(kvp => kvp.Key));
        }
        ...
        {
            _entityFoldouts.TryAdd(entry.Path, false);
            ...
            using (new EditorGUILayout.VerticalScope(BitterStyle.Card))
            {
                ...
                HandleEntityCardEvents(headerRect, entry, ref isExpanded);

                if (Event.current.type == EventType.Repaint)
                {
                    DrawEntityCardRepaint(headerRect, entry, isExpanded);
                }

                if (isExpanded)
                {
                    GUILayout.Space(5);
                    EditorGUI.DrawRect(EditorGUILayout.GetControlRect(false, 1), new Color(0, 0, 0, 0.1f));
                    GUILayout.Space(5);
        ...
        private void HandleEntityCardEvents(Rect headerRect, PrefabData entry, ref bool isExpanded)
        {
        ...
        private void DrawEntityCardRepaint(Rect headerRect, PrefabData entry, bool isExpanded)
        {
        ...
        private void DrawCleanInspector(MonoBehaviour component, GameObject prefab, string assetPath)
        {
            ...
            _componentFoldouts.TryAdd(persistentKey, false);
        ...
        private void DrawSerializedProperties(SerializedObject so)
        {
                ...
                foreach (var guid in guids)
                {
                    ...
                    if (asset != null)
                    {
                        ...
                        _allPrefabs.Add(new PrefabData
                        {
                            Name = asset.name,
                            Path = path,
                            Subfolder = string.IsNullOrEmpty(folderName) ? "Root" : folderName,
        ...
        private void ApplySortAndFilter()
        {
            ...
            orderedQuery = _currentSortMode switch
            {
                SortMode.Name => _sortAscending ? orderedQuery.ThenBy(p => p.Name) : orderedQuery.ThenByDescending(p => p.Name),
                SortMode.ComponentCount => _sortAscending ? orderedQuery.ThenBy(p => p.ComponentCount) : orderedQuery.ThenByDescending(p => p.ComponentCount),
                SortMode.Subfolder => _sortAscending
        ...
        private void ShowAddMenu(GameObject prefab)
        {
            ...
            foreach (var type in _availableProviderTypes.OrderBy(t => t.Name))
            {
                if (existing.Contains(type) || type.IsAbstract || type.IsGenericTypeDefinition) continue;
        ...
        private void AddComponentSafe(GameObject prefabAsset, Type type)
        {
            ...
            _entityFoldouts.TryAdd(path, true);
        }
    }
    ...
    public class PathsView
    {
        ...
        private void DrawCoreConfiguration()
        {
            using (new EditorGUILayout.VerticalScope(BitterStyle.Card))
            {
                ...
                if (EditorGUI.EndChangeCheck()) { /* Save trigger if needed */ }
            }
        }

        private void DrawConstantsDefinition()
        {
            using (new EditorGUILayout.VerticalScope(BitterStyle.Card))
            {
        ...
        {
            string baseSearchPath = string.IsNullOrEmpty(PathProject.RootPath)
                ? Application.dataPath
                : Path.Combine(Application.dataPath, PathProject.RootPath).Replace("\\", "/");
            ...
            var allDirectories = Directory.GetDirectories(baseSearchPath, "*", SearchOption.AllDirectories);
            int addedCount = 0;
            ...
            foreach (var realDir in allDirectories)
            {
                ...
                if (string.IsNullOrEmpty(relativeToRoot) || existingPaths.Contains(relativeToRoot))
                    continue;
        ...
        private void LoadCurrentConstants()
        {
            ...
            {
                var def = new PathDefinition { Name = Regex.Replace(field.Name, @"[^a-zA-Z0-9_]", "").ToUpper() };
        ...
        private void RefreshTree()
        {
            ...
            _rootNode = new PathNode("ROOT", "") { expanded = _owner.expandedPathKeys.Contains("ROOT") };
        ...
        private void DrawDefinitionRow(int index)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                var currentIndex = Mathf.Max(0, options.IndexOf(def.ParentName));
        ...
        private void GenerateScript()
        {
            ...
            foreach (var def in _pathDefinitions)
            {
                ...
                if (string.IsNullOrEmpty(def.ParentName) || def.ParentName == "None")
    ...
    public class SystemsView
    {
        ...
        public void SaveState()
        {
            ...
            _owner.expandedSystemKeys.AddRange(_foldouts.Where(kvp => kvp.Value).Select(kvp => kvp.Key));
        }

        public void Update()
        {
            if (Application.isPlaying && _autoRefresh && Time.realtimeSinceStartup - _lastRefreshTime > REFRESH_RATE)
            {
                _owner.Repaint();
        ...
        private void DrawHeader()
        {
            using (new EditorGUILayout.HorizontalScope(BitterStyle.Toolbar))
            {
                GUILayout.Label($"Active: {EcsReflectionHelper.GetSystemsCount()}", BitterStyle.SubHeaderLabel);
                GUILayout.FlexibleSpace();
        ...
        private void DrawSystemList()
        {
            ...
            var filtered = allSystems
                .Where(s => string.IsNullOrEmpty(_searchFilter) || s.GetType().Name.Contains(_searchFilter, StringComparison.OrdinalIgnoreCase))
        ...
        private void DrawPriorityGroup(Priority priority, List<IEcsSystem> systems)
        {
            ...
            EditorGUI.DrawRect(headerRect, GetPriorityColor(priority) * 0.3f);
        ...
        {
            BitterStyle.DrawBadge(type.Name.Replace("IEcs", ""), GetInterfaceColor(type));
        }

        private void ExpandCollapseAll(bool expand)
        {
            foreach (var key in _foldouts.Keys.ToList()) _foldouts[key] = expand;
        }
    ...
    public class BitterECSControlPanel : EditorWindow, ISerializationCallbackReceiver
    {
        ...
        private PathsView _pathsView; [MenuItem("BitterECS/Control Panel", priority = 0)]
        ...
        private void DrawToolbar()
        {
            ...
            EditorGUI.DrawRect(EditorGUILayout.GetControlRect(false, 1), new Color(0, 0, 0, 0.3f));
        }
