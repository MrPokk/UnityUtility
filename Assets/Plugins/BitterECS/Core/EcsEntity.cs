...
namespace BitterECS.Core
{
    public readonly struct EcsEntity : IEquatable<EcsEntity>
    {
        ...
        public void AddFrame<T>(in T component = default) where T : new() { if (!IsAlive) return; Add(component); Remove<T>(); }
        public void AddFrame<T>(Action action, in T component = default) where T : new() { if (!IsAlive) return; Add(component); action(); Remove<T>(); }
        ...
        {
            if (Has<T>()) Get<T>() = component;
            else
            {
                Presenter.GetPool<T>().Add(Id, component);
                Presenter.SetMaskBit(Id, ComponentTypeId<T>.Id);
            }
        }

        public void Set<T>(RefAction<T> modifier) where T : new() => modifier(ref Get<T>());
        public delegate void RefAction<T>(ref T component) where T : new(); [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Has<T>() where T : new() => !IsNull && Presenter.GetEntityMask(Id).Has(ComponentTypeId<T>.Id);
        ...
        public void Remove<T>() where T : new()
        {
            if (Has<T>())
            {
                ...
                Presenter.RemoveMaskBit(Id, ComponentTypeId<T>.Id);
            }
        }
        ...
        public bool TryGetProvider<T>(out T provider) where T : class, ILinkableProvider => (provider = GetProvider<T>()) != null;
